...
orthNeighbors :: (Dimensions [w, h]) => Grid [w, h] (Coord [w, h]) -> Orth (Coord [w, h])
orthNeighbors g = Orth
  { up = g `index` Coord [1, 0]
  , right = g `index` Coord [2, 1]
  , down = g `index` Coord [1, 2]
  , left = g `index` Coord [0, 1]
  }
